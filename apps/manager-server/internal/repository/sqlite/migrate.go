...
func Migrate(db *sql.DB) error {
	...
	statements := []string{
		...
		`create table if not exists codex_inspection_results (
			...
			credits_observed integer not null default 0,
...
func ensureCodexInspectionResultColumns(db *sql.DB) error {
	...
	columns := []struct {
		...
		{name: "credits_observed", definition: "integer not null default 0"},
		{name: "credits_balance", definition: "real"},
		{name: "credits_has_credits", definition: "integer"},
		{name: "credits_unlimited", definition: "integer"},
