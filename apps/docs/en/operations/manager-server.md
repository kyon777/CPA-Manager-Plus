...
| `TOKEN_ACQUISITION_BASE_URL`             | `https://401.kyon888.xyz`                                   | Server-only TokenAcquisition v1 endpoint; the browser never calls this service directly.                                                                                                                                          |
| `TOKEN_ACQUISITION_API_KEY`              | empty                                                       | Optional administrator API key; prefer the file variable below in production.                                                                                                                                                     |
| `TOKEN_ACQUISITION_API_KEY_FILE`         | `/run/secrets/token_acquisition_api_key`                    | Server-only TokenAcquisition administrator API key file; it is never persisted in SQLite, logs, or browser responses.                                                                                                              |
