...
| `TOKEN_ACQUISITION_BASE_URL`             | `https://401.kyon888.xyz`                                   | 服务端 TokenAcquisition v1 地址；浏览器不会直接访问该服务。                                                                                               |
| `TOKEN_ACQUISITION_API_KEY`              | 空                                                          | 可选管理员 API Key；生产部署优先使用下方文件变量。                                                                                                         |
| `TOKEN_ACQUISITION_API_KEY_FILE`         | `/run/secrets/token_acquisition_api_key`                    | 服务端 TokenAcquisition 管理员 API Key 文件；不会写入 SQLite、日志或前端响应。                                                                              |
